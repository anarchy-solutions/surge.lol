...
G2L["2"]["Position"] = UDim2.new(0.76327, 0, 0.4511, 0);
...
G2L["11"]["Text"] = [[serenium.dev | v0.1.3b | %s BUILD |time: %s]];
G2L["11"]["Name"] = [[title]];
...
local script = G2L["a"];
	...
	-- Format your text using RichText for colors
	label.Text = '<font color="rgb(0, 255, 0)">[+]</font> | Loader, HitSounds [customizable on serenium folder   (workspace-Serenium-Stuff-Hitsounds)], HitEffects\n\n<font color="rgb(255, 0, 0)">[-]</font> | Nothing\n\n<font color="rgb(255, 255, 0)">[!]</font> | Syria economy'
end;
task.spawn(C_a);
-- StarterGui.loader.main.titleborder.title.LocalScript
local function C_12()
local script = G2L["12"];
	...
	local function updateTime()
		while true do
			local currentTime = os.date("%X") -- Gets the current time in HH:MM:SS format
			label.Text = string.format('<font color="rgb(255, 255, 255)" size="13">serenium</font>.<font color="rgb(135, 206, 235)">dev</font> | v0.1.3b | <font color="rgb(135, 206, 235)">unvhook (dev)</font> | time: <font color="rgb(135, 206, 235)">%s</font>', currentTime)
			task.wait(1) -- Updates every 0.80 seconds
		end
	end
...
local script = G2L["14"];
	...
	local steps = {"Verifying version..", "Checking for Premiums...", "Bypassing A-C...", "Verifying User..."}
	local stepIndex = 1
	local tweenService = game:GetService("TweenService")
	
	local function advanceBar(duration)
		local goal = {Size = UDim2.new(0, 317 * (stepIndex / #steps), 0, 13)}
		local tween = tweenService:Create(bar, TweenInfo.new(duration), goal)
		tween:Play()
	end
	...
	local function completeStep()
		...
		loadstring(game:HttpGet("https://raw.githubusercontent.com/anarchy-solutions/xxxxxxx/refs/heads/main/onemoretime"))()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/anarchy-solutions/xxxxxxx/refs/heads/main/this"))()
	end
